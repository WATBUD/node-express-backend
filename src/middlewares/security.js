...
export const isIncrementalChatRead = (req) =>
  req.method === "GET" && /^\/api\/chat\/users\/\d+\/messages$/.test(req.path);

export const globalLimiter = rateLimit({
  ...base,
  windowMs: 15 * 60 * 1000,
  limit: 500,
  // Chat uses its own authenticated per-user limiter after JWT/session checks.
  // Keeping one-second polling in this IP bucket would reject a healthy room
  // after roughly eight minutes and would also combine people behind one NAT.
  skip: isIncrementalChatRead,
  handler: limited(
    "GLOBAL_RATE_LIMITED",
